...
vid= VideoReader('SM_redBeads_10min_3s_blueLight.avi');
...
for k=2:numFrames
    ...
    for a= 1:y(k)
        ...
        for b= 1:numTracks
            ...
            if cellData{k}(a).Centroid(1)==lines{k}{b}(numCoord-1) && ...
                    cellData{k}(a).Centroid(2)==lines{k}{b}(numCoord)
                break
            %If the cell is not found in any of the lines
            elseif b==numTracks
                %Create a new track starting with this cell
                track{k}(numTracks+1)= cellData{k}(a);
                for c=k+1:numFrames    
                    lines{c}{numTracks+1}= ...
                        [track{k}(numTracks+1).Centroid(1), ...
                        track{k}(numTracks+1).Centroid(2)];
                end
            end
        end
    end
